...
class AdminMeetBiz extends BaseBiz {
	...
	static async updateMeetStyleSet(meetId, styleSet, that) {
		...
		// 图片上传到云空间
		if (styleSet.pic)
			pic = await chelper.transTempPicOne(pic, setting.MEET_PIC_PATH, meetId, false);
		...
		try {
			...
			await chelper.callCloudSumbit('admin/meet_update_style', params);
			that.setData({
	...
	static async updateMeetCotnentPic(meetId, content, that) {
		...
		// 图片上传到云空间
		imgList = await chelper.transTempPics(imgList, setting.MEET_PIC_PATH, meetId);
		...
		try {
			// 更新数据 从promise 里直接同步返回
			await chelper.callCloudSumbit('admin/meet_update_content', params);
			that.setData({
				formContent: content
			});
		} catch (e) {
