...
class ANewsBiz {
	...
	static async updateNewsPic(newsId, imgList) {
		...
		// 图片上传到云空间
		imgList = await chelper.transTempPics(imgList, setting.NEWS_PIC_PATH, newsId);
		...
		try {
			...
			let res = await chelper.callCloudSumbit('admin/news_update_pic', params);
	...
	static async updateNewsCotnentPic(newsId, content, that) {
		...
		try {
			// 更新数据 从promise 里直接同步返回
			await chelper.callCloudSumbit('admin/news_update_content', params);
			that.setData({
				formContent: content
			});
		} catch (e) {
	...
	static async initFormData(id = '') {
		let cateIdOptions = await ANewsBiz.getCateList();
