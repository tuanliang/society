...
module.exports = Behavior({
	...
	methods: {
		...
		_loadDetail: async function (e) {
			...
			let user = await chelper.callCloudData('passport/my_detail', {}, opts);
			if (!user) {
				this.setData({
					isLoad: true,
					formName: '',
		...
		bindGetPhoneNumber: async function (e) {
			if (e.detail.errMsg == "getPhoneNumber:ok") {
				...
				await chelper.callCloudSumbit('passport/phone', params, opt).then(res => {
					let phone = res.data;
		...
		bindSubmitTap: async function (e) {
			try {
				...
				if (mobile.length != 11) return pHelper.showModal('请填写正确的手机号码');
				...
				await chelper.callCloudSumbit('passport/edit_base', data, opts).then(res => {
					let callback = () => {
						wx.navigateBack();
					}
					pHelper.showSuccToast('提交成功', 1500, callback);
				});
			} catch (err) {
				console.error(err);
			}
