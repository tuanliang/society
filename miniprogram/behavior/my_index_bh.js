	...
	methods: {
		...
		_loadTodayList: async function () {
			try {
				...
				await chelper.callCloudSumbit('my/my_join_someday', params, opts).then(res => {
					this.setData({
		...
		setTap: function (e, skin) {
			let itemList = ['清除缓存', '后台管理','后台账号:admin,密码:shiyi3759551'];
			wx.showActionSheet({
				itemList,
				success: async res => {
					...
					if (idx == 0) {
						cacheHelper.clear();
						pHelper.showNoneToast('清除缓存成功');
					}

					if (idx == 1) {
						pHelper.setSkin(skin);
						if (setting.IS_SUB) {
          ...
          if (idx == 2) {
						...
						pHelper.showNoneToast('账号：admin，密码：shiyi3759551');
					}
				},
				fail: function (res) {}
			})
