...
module.exports = Behavior({
	...
	methods: {
		...
		_loadDetail: async function () {
			...
			let meet = await chelper.callCloudData('meet/view', params, opt);
			if (!meet) {
				this.setData({
					isLoad: null
				})
		...
		bindJoinTap: async function (e) {
			let dayIdx = pHelper.dataset(e, 'dayidx');
			let timeIdx = pHelper.dataset(e, 'timeidx');
			...
			if (time.error) {
				if (time.error.includes('预约'))
					return pHelper.showModal('该时段' + time.error + '，换一个时段试试吧！');
				else
					return pHelper.showModal('该时段预约' + time.error + '，换一个时段试试吧！');
			}
			...
			let callback = async () => {
				try {
					...
					await chelper.callCloudSumbit('meet/before_join', params, opts).then(res => {
						wx.navigateTo({
