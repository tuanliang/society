	...
	methods: {
		...
		onLoad: async function (options) {
			if (options && options.scene) {
				...
				try {
					await chelper.callCloudSumbit('my/my_join_checkin', params, opts).then(res => {
						let cb = () => {
							wx.reLaunch({
								url: pHelper.fmtURLByPID('/pages/my/index/my_index'),
							});
						}
						pHelper.showModal(res.data.ret, '温馨提示', cb);
					});
				} catch (err) {
					console.error(err);
				}
			} else {
				pHelper.showModal('签到码扫描错误，请关闭本小程序，使用「微信›扫一扫」重新扫码');
			}
		},
