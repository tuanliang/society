...
module.exports = Behavior({
	...
	methods: {
		...
		onLoad: async function (options) {
			...
			if (!pHelper.getOptions(this, options, 'timeMark')) return;
		...
		_loadDetail: async function () {
			...
			let meet = await chelper.callCloudData('meet/detail_for_join', params, opt);
			if (!meet) {
				this.setData({
					isLoad: null
				})
		...
		bindSubmitCmpt: async function (e) {
			...
			let callback = async () => {
				try {
					...
					await chelper.callCloudSumbit('meet/join', params, opts).then(res => {
						let content = '预约成功！'
						...
						wx.showModal({
							...
							success() {
								let ck = () => {
									wx.reLaunch({
										url: pHelper.fmtURLByPID('/pages/my/join_detail/my_join_detail?flag=home&id=' + joinId),
									})
								}
								ck();
							}
