...
Component({
	...
	methods: {
			...
			wx.chooseImage({
				...
				success(res) {
					...
					if (!contentCheckHelper.imgTypeCheck(path)) {
						wx.hideLoading();
						return pHelper.showNoneToast('只能上传png、jpg、jpeg格式', 3000);
					}
					...
					if (!contentCheckHelper.imgSizeCheck(size, imageMaxSize)) {
						wx.hideLoading();
						return pHelper.showModal('图片大小不能超过 ' + maxSize + '兆');
					}
		...
		bidnDeleteNodeTap: function (e) {
			let idx = pHelper.dataset(e, 'idx');
			let nodeList = this.data.nodeList;
			if (this.data.nodeList.length == 1) return pHelper.showNoneToast('至少需要一个内容框');
		...
		bindUpTap: function (e) {
			...
			pHelper.anchor('editor-node-' + (idx - 1), this);
			this.setGlow(idx - 1);
		},
		bindTopTap: function (e) {
			...
			pHelper.anchor('editor-node-0', this);
			this.setGlow(0);
		},
		bindBottomTap: function (e) {
			...
			pHelper.anchor('editor-node-' + (nodeList.length - 1), this);
