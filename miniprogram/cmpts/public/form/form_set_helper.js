...
function checkForm(fields, forms) {
	...
	for (let k in fields) {
		...
		if (fields[k].must && type != 'switch' && (!helper.isDefined(val) || val.length == 0)) {
			...
			return pHelper.showModal(hintOprt + '' + title);
		}
		...
		if (type == 'line' || type == 'multi' || type == 'number' || type == 'digit') {
			if (val.length > fields[k].len) {
				...
				return pHelper.showModal(title + ' 字数太多，精简一下吧');
			}
		}
		...
		switch (type) {
			case 'mobile': {
				if (val.length > 0 && val.length != 11) {
					...
					return pHelper.showModal('请填写正确的 ' + title);
				}
				break;
			}
			case 'switch': {
			...
			case 'checkbox': {
				if (val.length > 0 && val.length < fields[k].checkBoxLimit) {
					...
					return pHelper.showModal(title + ' 至少选中' + fields[k].checkBoxLimit + '项');
				}
				break;
			}
			case 'date': {
