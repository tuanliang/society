...
Page({
	...
	bindDelTap: function (e) {
		...
		pHelper.showConfirm('确定要删除当前字段吗？', callback);
	},

	bindSubmitTap: function (e) {
		if (this.data.formTitle.length < 1) return pHelper.showModal('字段名称必填哦');
		if (this.data.formTitle.length > 60) return pHelper.showModal('字段名称必不能超过60个字');

		if (this.data.formDesc.length > 30) return pHelper.showModal('填写说明不能超过30个字');

		if (this.data.formType.length < 1) return pHelper.showModal('字段填写类型必须选择哦');
		...
		if (formType == 'select' || formType == 'checkbox') {
			...
			for (let k in formSelectOptions) {
				if (formSelectOptions[k].length < 1) {
					return pHelper.showModal('选项' + (Number(k) + 1) + '还没填哦');
				}

				if (formSelectOptions[k].length > 30) {
					return pHelper.showModal('选项' + (Number(k) + 1) + '不能超过30个字，精简一点!');
				}
			}
	...
	bindDelSelectOptionsTap: function (e) {
		let formSelectOptions = this.data.formSelectOptions;
		if (formSelectOptions.length <= 2) return pHelper.showModal('至少2个选项');
		...
		pHelper.showConfirm('确定删除该项吗？', callback);
	},

	bindAddSelectOptionsTap: function (e) {
		let formSelectOptions = this.data.formSelectOptions;
		if (formSelectOptions.length >= 20) return pHelper.showModal('最多可以添加20个选项');
