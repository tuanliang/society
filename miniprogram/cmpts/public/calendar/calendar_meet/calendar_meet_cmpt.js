	...
	methods: {
		...
		bindDayMultiTap(e) { // 多选天点击
			...
			if (!this.data.selectTimeout) {
				...
				if (day < now)
					return pHelper.showNoneToast('不能编辑过往的日期');
			}

			// 是否有预约判断
			if (this.data.hasJoinDays.includes(day)) {
				return pHelper.showModal('该日期已有用户预约/预约待审核，不可直接取消；如果确要取消，请先删除有预约的时段');
			}
		...
		// ListTouch触摸开始
		listTouchStart(e) {
			pHelper.listTouchStart(e, this);
		},

		// ListTouch计算方向
		listTouchMove(e) {
			pHelper.listTouchMove(e, this);
		},
