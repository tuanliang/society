...
Component({
	...
	methods: {
				...
				success: async (res) => {
					...
					for (let k = 0; k < res.tempFiles.length; k++) {
						...
						if (!contentCheckHelper.imgSizeCheck(size, imageMaxSize)) {
							wx.hideLoading();
							return pHelper.showNoneToast('单张图片大小不能超过 ' + this.data.imgUploadSize + 'M', 3000);
						}
						...
						if (this.data.isCheck) {
							...
							if (!check) {
								wx.hideLoading();
								return pHelper.showNoneToast('存在不合适的图片, 已屏蔽', 3000);
							}
						}
		...
		catchDelImgTap: function (e) {
			...
			pHelper.showConfirm('确定要删除该图片吗？', callback);
		},
