...
Page({
	...
	bindFormSubmit: async function () {
		...
		try {
			...
			aboutPic = await chelper.transTempPics(aboutPic, setting.SETUP_PIC_PATH, '');

			data.aboutPic = aboutPic;
			await chelper.callCloudSumbit('admin/setup_about', data).then(res => {
				let callback = () => {
					wx.navigateBack({
						delta: 0,
					});
				}
				pHelper.showSuccToast('编辑成功', 1500, callback);
			});
