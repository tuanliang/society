...
Page({
	...
	onLoad: async function (options) {
		...
		pHelper.getOptions(this, options);
	...
	_loadDetail: async function () {
		...
		pHelper.formSetBarTitleByAddEdit(id, '后台-活动/预约');
		...
		let meet = await chelper.callCloudData('admin/meet_detail', params, opt);
	...
	bindFormAddSubmit: async function () {
		pHelper.formClearFocus(this);

		if (!ABiz.isAdmin(this)) return;

		let data = this.data;
		if (data.formTitle.length <= 0) return pHelper.formHint(this, 'formTitle', '请填写「标题」');

		if (data.formTypeId.length <= 0) return pHelper.formHint(this, 'formTypeId', '请选择「分类」');

		if (data.formStyleSet.pic.length <= 0) {
			pHelper.anchor('formStyleSet', this);
			return pHelper.formHint(this, 'formStyleSet', '封面图片未设置');
		}
		if (data.formDaysSet.length <= 0) {
			pHelper.anchor('formDaysSet', this);
			return pHelper.formHint(this, 'formDaysSet', '请配置「可预约时段」');
		}
		if (data.formFormSet.length <= 0) return pHelper.showModal('请至少设置一项「用户填写资料」');

		if (data.contentDesc.includes('未填写'))
			return pHelper.formHint(this, 'formContent', '请填写「详细介绍」');
		...
		try {
			// 先创建，再上传 
			let result = await chelper.callCloudSumbit('admin/meet_insert', data);
	...
	bindFormEditSubmit: async function () {
		...
		try {
			...
			// 先修改，再上传 
			await chelper.callCloudSumbit('admin/meet_edit', data);
			...
			let callback = async function () { 
				...
				pHelper.modifyPrevPageListNodeObject(meetId, node);
				wx.navigateBack();

			}
			pHelper.showSuccToast('编辑成功', 2000, callback);
