...
Page({
	...
	bindTimeAddTap: function (e) {
		let idx = pHelper.dataset(e, 'idx');
		let days = this.data.days;

		if (days[idx].times.length >= 20) return pHelper.showModal('最多可以添加20个时段');
	...
	bindCancelMeetJoinCmpt: async function (e) { //取消已有预约
		...
		try {
			...
			await chelper.callCloudSumbit('admin/meet_cancel_time_join', params, opt).then(res => {
				let callback = () => {
				...
				pHelper.showSuccToast('取消成功', 1500, callback);
			})
		} catch (err) {
			console.log(err);
		};
	...
	bindTimeDelTap: function (e) {
		let idx = pHelper.dataset(e, 'idx');
		let timesIdx = pHelper.dataset(e, 'timesidx');
		let days = this.data.days;
		...
		if (node.stat.succCnt || node.stat.waitCheckCnt) {
			...
			pHelper.showConfirm('该时段已有「' + (node.stat.succCnt + node.stat.waitCheckCnt) + '人」预约/预约待审核，若选择删除则将取消所有预约，请仔细确认！ 若不想取消，可以选择停止该时段', callback);
		} else {
			let callback = () => {
			...
			pHelper.showConfirm('是否要删除该时间段？', callback);
		}
	},
	...
	bindTimeStatusSwitch: function (e) {
		...
		if (status == 0) {
			...
			pHelper.showConfirm('是否要停止该时间段的预约？停止后，已有预约记录仍将保留', yes);
		}
	...
	bindDaysTimeStartCmpt: function (e) {
		...
		if (start >= end) return pHelper.showModal('开始时间不能大于等于结束时间');
	...
	switchModel: function (e) {
		pHelper.switchModel(this, e, 'bool');
	},

	bindSaveTempCmpt: async function (e) {
		try {
			let name = this.data.formTempName;
			if (name.length <= 0) return pHelper.showNoneToast('请填写模板名称');
			if (name.length > 20) return pHelper.showNoneToast('模板名称不能超过20个字哦');

			let days = this.data.days;
			let times = days[this.data.curIdx].times;
			if (times.length <= 0) return pHelper.showNoneToast('至少需要包含一个时段');
			if (times.length > 20) return pHelper.showNoneToast('时段不能超过20个');
			...
			await chelper.callCloudSumbit('admin/temp_insert', params, opt).then(res => {
				pHelper.showSuccToast('保存成功');
				this.setData({
					saveTempModalShow: false,
					formTempName: '',
				});
	...
	bindShowTimeLimitModalTap: function (e) {
		let curIdx = pHelper.dataset(e, 'idx');
		let curTimesIdx = pHelper.dataset(e, 'timesidx');
	...
	_selectTemp: function (e) {
		...
		if (this._checkHasJoinCnt(this.data.days[curIdx].times)) {
			return pHelper.showModal('该日已有用户预约/预约待审核，不能选用模板。若确定要选用模板，请先删除有预约的时段');
		}
	...
	_saveTempModal: function (e) {
		let curIdx = pHelper.dataset(e, 'idx');
		let days = this.data.days;
		if (days[curIdx].times.length <= 0) return pHelper.showModal('该日期下没有设置时段，无法保存为模板，请先添加时段');
		this.setData({
	...
	_copyDaySetToAll: function (e) { //  复制到所有
		...
		pHelper.showConfirm('确认将「' + day + '」下的时段设置复制到其他日期下吗? (原有时段将被清除，如已有预约记录则该日的所有时段将不被修改)', callback);
	},
		...
		wx.showActionSheet({
			...
			success: async res => {
				...
				if (idx == 2) { //  删除
					let curIdx = pHelper.dataset(e, 'idx');
					if (this._checkHasJoinCnt(this.data.days[curIdx].times)) {
						return pHelper.showModal('该日已有用户预约/预约待审核，不能直接删除。若确定要删除，请先删除有预约的时段')
					}
					let callback = () => {
						let days = this.data.days;
					...
					pHelper.showConfirm('确认删除该日期吗?', callback);
				}
