...
Page({
	...
	bindAllLimitSetCmpt: async function (e) {
		...
		try {
			...
			await chelper.callCloudSumbit('admin/temp_edit', params, opts).then(res => {
				this.setData({
					temps: res.data,
					curTimeModalShow: false,
	...
	_loadList: async function () {
		try {
			...
			await chelper.callCloudSumbit('admin/temp_list', {}, opts).then(res => {
				this.setData({
	...
	bindSelectTap: function (e) {
		...
		pHelper.showConfirm('确认要选用模板 「' + name + '」配置到日期 「' + day + '」下吗?', callback);
	},
	...
	_delTemp: async function (curIdx, id) {
		try {
			...
			await chelper.callCloudSumbit('admin/temp_del', params, opts).then(res => {
		...
		wx.showActionSheet({
			...
			success: async res => {
				...
				if (idx == 0) { // 删除
					...
					pHelper.showConfirm('确认要删除模板 「' + name + '」吗?', callback);
				}
				if (idx == 1) {
					this.setData({
						curIdx,
